...
- [ ] `kernel/cli/.build/release/wub sweep --json` (confirm JSON output)
- [ ] `kernel/cli/.build/release/wub plan --json` (confirm JSON output)
- [ ] `kernel/cli/.build/release/wub setup --show-manual` (confirm no automated steps fail)
- [ ] `kernel/cli/.build/release/wub profile use wub` (confirm operator/notes/WUB_CONFIG.json written)
- [ ] Station gating check: `kernel/cli/.build/release/wub station status --format json --no-write-report`
