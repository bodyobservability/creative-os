...
**Goal:** Spoken phrase → exactly one MIDI event on the dedicated bus `HVLIEN_VOICE` (IAC).  
...
→ 3) `sendmidi` CLI  
→ 4) **IAC Driver** port `HVLIEN_VOICE`
...
5. Add a port named: **HVLIEN_VOICE**

### Acceptance
- A port named `HVLIEN_VOICE` exists and is online.
...
Send a CC and a Note to `HVLIEN_VOICE` and confirm the port receives it (via MIDI Monitor or Ableton MIDI indicator).
...
- Execute a shell command that sends the MIDI event to `HVLIEN_VOICE`

Example structure:
- **Execute Shell Script**:
  - `sendmidi dev "HVLIEN_VOICE" ch 1 cc 14 96`
  - or `sendmidi dev "HVLIEN_VOICE" ch 1 on 60 127`
...
1) **Terminal** → sendmidi → IAC (`HVLIEN_VOICE`)  
...
- Keep all runtime output on `HVLIEN_VOICE` only.
...
- `specs/voice_runtime/voice_runtime_triggers.v9.yaml`
- `specs/voice_runtime/voice_runtime_triggers.v9.schema.json`
...
- All events arrive on `HVLIEN_VOICE` consistently.
