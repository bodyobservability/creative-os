# v9.4 — Runtime Mapping Validator

Adds a validator that produces a receipt proving your Ableton set is *ready* for v9 runtime triggers.

Command:
```bash
hvlien vrl validate --mapping specs/voice_runtime/v9_3_ableton_mapping.v1.yaml --regions tools/automation/swift-cli/config/regions.v1.json
```

What it checks (best-effort):
- CoreMIDI destination exists matching the mapping `midi.bus` (default: HVLIEN_VOICE)
