  ...
  for i in 0..<n {
    ...
    for ei in 0..<ec {
      ...
      for si in 0..<MIDIEntityGetNumberOfSources(ent) {
        ...
        ins.append(MidiEndpoint(id: "in_\(i)_\(ei)_\(si)", displayName: midiString(src, kMIDIPropertyName) ?? "Source", normName: StudioNormV1.normNameV1(midiString(src, kMIDIPropertyName)), direction: "input", uniqueId: midiInt(src, kMIDIPropertyUniqueID)))
      }
      for di in 0..<MIDIEntityGetNumberOfDestinations(ent) {
        let dst = MIDIEntityGetDestination(ent, di)
        outs.append(MidiEndpoint(id: "out_\(i)_\(ei)_\(di)", displayName: midiString(dst, kMIDIPropertyName) ?? "Dest", normName: StudioNormV1.normNameV1(midiString(dst, kMIDIPropertyName)), direction: "output", uniqueId: midiInt(dst, kMIDIPropertyUniqueID)))
      }
    }
    devices.append(ControllerDevice(id: "dev_\(UUID().uuidString)", displayName: name, normName: StudioNormV1.normNameV1(name), manufacturer: mfr, model: model, endpointsIn: ins, endpointsOut: outs))
  }
