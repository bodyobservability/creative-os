  ...
  func run() async throws {
    ...
    if !FileManager.default.fileExists(atPath: artifactIndex) {
      ...
      if let (missing, placeholder, stale) = artifactCounts(path: artifactIndex) {
        ...
        if pending > 0 {
          cmds.append("wub assets export-all --anchors-pack \(anchorsPackHint) --overwrite")
          ...
          cmds.append("wub drift check --anchors-pack-hint \(anchorsPackHint)")
        }
      } else {
    ...
    if !anchorsOk {
      cmds.append("wub validate-anchors --regions-config tools/automation/swift-cli/config/regions.v1.json --pack \(anchorsPackHint)")
    ...
    if let rd = chosenRunDir, let drift = findLatestJSON(withPrefix: "drift_report", inDir: rd) {
      if let st = readStatus(fromJSON: drift) {
        ...
        if st == "fail" {
          cmds.append("wub drift plan --anchors-pack-hint \(anchorsPackHint)")
          cmds.append("wub drift fix --anchors-pack-hint \(anchorsPackHint)")
        }
      } else {
