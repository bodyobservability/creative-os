...
struct Station: AsyncParsableCommand {
  ...
  struct Certify: AsyncParsableCommand {
    ...
    func run() async throws {
      ...
      let profPath = profilePath ?? WubDefaults.profileSpecPath("station/profiles/\(profile).yaml")
    ...
    private func loadProfile(yaml: String) throws -> ProfileCfg {
      ...
      let sessionPath = (get(["session","profile"]) as? String) ?? WubDefaults.profileSpecPath("session/profiles/bass_v1.yaml")
