...
struct SimpleInventoryIndex {
  ...
  init(inventory: InventoryDoc) {
    ...
    for it in inventory.items {
      s.insert(HVLIENNormV1.normNameV1(it.displayName))
    }
    normNames = s
  }
  func has(_ name: String) -> Bool {
    let n = HVLIENNormV1.normNameV1(name)
...
enum RackRecommend {
  ...
  static func recommend(manifest: RackPackManifestV1, inventory: InventoryDoc, recsPath: String?) -> [Recommendation] {
    ...
    for r in manifest.racks {
      ...
      if missing.contains(where: { HVLIENNormV1.normNameV1($0).contains("serum") }) {
        suggestions += (tagToSug["synth"] ?? ["Serum 2 (AU)","Wavetable"])
      }
      if missing.contains(where: { HVLIENNormV1.normNameV1($0).contains("utility") }) {
