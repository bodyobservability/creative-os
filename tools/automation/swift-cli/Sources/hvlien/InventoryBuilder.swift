...
final class InventoryBuilderV1 {
  ...
  func ingest(_ s: InventorySighting) {
    let norm = HVLIENNormV1.normNameV1(s.displayName)
    guard norm != "__invalid__" else { return }
    let key = HVLIENNormV1.dedupeKeyV1(kind: s.kind, format: s.format, vendor: s.vendor, norm: norm)
    ...
    let stable = HVLIENNormV1.stableKeyV1(kind: s.kind, format: s.format, vendor: s.vendor, norm: norm)
    raw.append(InventoryItem(
      ...
      format: s.format,
      vendor: s.vendor,
      tags: s.tags.map { HVLIENNormV1.normNameV1($0) }.filter { $0 != "__invalid__" },
      browserPath: [],
  ...
  func finalize() -> [InventoryItem] {
    ...
    return merged.map { it in
      var o = it
      o.stableKey = HVLIENNormV1.stableKeyV1(kind: it.kind, format: it.format, vendor: it.vendor, norm: it.normName)
      return o
    }
  }
}
