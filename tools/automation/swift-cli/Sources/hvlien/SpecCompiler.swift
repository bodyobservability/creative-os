...
enum SpecCompiler {
    ...
    for t in tracks {
      ...
      for (i, devAny) in chain.enumerated() {
        ...
        out.append(DeviceRequest(
          ...
          matchMode: mm,
          required: required,
          tags: tags.map { HVLIENNormV1.normNameV1($0) }.filter { $0 != "__invalid__" },
  ...
  private static func impliedPackChecks(tagSet: Set<String>, packSignaturesPath: String?) throws -> [PackCheckRequest] {
    ...
    for pack in doc.packs {
      let any = Set((pack.impliedByTagsAny ?? []).map { HVLIENNormV1.normNameV1($0) }.filter { $0 != "__invalid__" })
      let all = Set((pack.impliedByTagsAll ?? []).map { HVLIENNormV1.normNameV1($0) }.filter { $0 != "__invalid__" })
