...
struct VRL: AsyncParsableCommand {
  ...
  struct Validate: AsyncParsableCommand {
    ...
    var mapping: String = "specs/voice_runtime/v9_3_ableton_mapping.v1.yaml"
    ...
    func run() async throws {
      ...
      let midiBus = ((mappingRoot["midi"] as? [String: Any])?["bus"] as? String) ?? "HVLIEN_VOICE"
    ...
    private func ocrContainsAll(frame: CGImage, regions: RegionsV1, regionId: String, tokens: [String], runDir: URL, dump: Bool, dumpPrefix: String) async throws -> OCRAllResult {
      ...
      var missing: [String] = []
      for t in tokens {
        let nt = HVLIENNormV1.normNameV1(t)
