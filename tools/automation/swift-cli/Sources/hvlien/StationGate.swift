...
enum StationGate {
  ...
  /// v1.7.18: Evaluate station context by calling `hvlien station status --format json --no-write-report`.
  ...
  static func evaluate(anchorsPackHint: String?, force: Bool = false) -> GateResult {
      ...
      if state == "blocked" {
        return GateResult(decision: .refuse,
                          ...
                          suggestedNext: [
                            ...
                            "Run: hvlien doctor --modal-test detect --allow-ocr-fallback"
                          ])
      }
      if state == "exporting" {
        return GateResult(decision: .refuse,
    ...
    return GateResult(decision: .warn_force,
                      ...
                      suggestedNext: [
                        "Run: hvlien station status --format human",
                        "Or pass --force to proceed anyway"
                      ])
  }
  ...
  private static func readStationStatusJSON() -> StationStatusEnvelope? {
    ...
    let cmd = "hvlien station status --format json --no-write-report"
