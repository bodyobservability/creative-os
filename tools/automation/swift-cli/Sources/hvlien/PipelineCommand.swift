...
struct Pipeline: AsyncParsableCommand {
  ...
  struct CutProfile: AsyncParsableCommand {
    ...
    @Option(name: .long, help: "Baseline path (canonical). If omitted, uses specs/sonic/baselines/<rack>/<macro>.baseline.v1.json")
    var baseline: String?
    ...
    func run() async throws {
      ...
      let basePath = baseline ?? "specs/sonic/baselines/\(rackId)/\(macro).baseline.v1.json"
