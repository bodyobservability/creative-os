...
final class WubConfigTests: XCTestCase {
  ...
  func testConfigCreatedFromFirstProfile() throws {
    ...
    try fm.createDirectory(at: tempDir.appendingPathComponent(WubPaths.operatorProfilesDir, isDirectory: true), withIntermediateDirectories: true)
    try fm.createDirectory(at: tempDir.appendingPathComponent(WubPaths.operatorNotesDir, isDirectory: true), withIntermediateDirectories: true)

    let profilePath = tempDir.appendingPathComponent(WubPaths.operatorProfilesDir).appendingPathComponent("test.profile.yaml")
    try "id: test\nintents: []\npolicies: {}\nrequirements: {}\npacks: []\n"
  ...
  func testWubProfileUsePersistsSelection() async throws {
    ...
    let profilePath = tempDir.appendingPathComponent(WubPaths.operatorProfilesDir).appendingPathComponent("hvlien.profile.yaml")
    try "id: hvlien\nintents: []\npolicies: {}\nrequirements: {}\npacks: []\n"
      .write(to: profilePath, atomically: true, encoding: .utf8)
  ...
  func testActivePackSelectionIncludedInDesiredState() throws {
    ...
    try fm.createDirectory(at: tempDir.appendingPathComponent(WubPaths.operatorProfilesDir, isDirectory: true), withIntermediateDirectories: true)
    try fm.createDirectory(at: tempDir.appendingPathComponent(WubPaths.operatorPacksDir).appendingPathComponent("hvlien-defaults", isDirectory: true), withIntermediateDirectories: true)
    try fm.createDirectory(at: tempDir.appendingPathComponent(WubPaths.operatorNotesDir, isDirectory: true), withIntermediateDirectories: true)
    ...
    let packPath = tempDir.appendingPathComponent(WubPaths.operatorPacksDir).appendingPathComponent("hvlien-defaults/pack.yaml")
