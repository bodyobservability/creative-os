...
enum ResolvePhase {
  static func runResolve(specPath: String,
    ...
    for r in compiled.deviceRequests {
      let found = inv.items.contains { StudioNormV1.normNameV1($0.displayName) == StudioNormV1.normNameV1(r.primary) }
    ...
    for c in compiled.controllerRequests {
      let tokens = c.expectedNameContains.map { StudioNormV1.normNameV1($0) }
    ...
    for pc in compiled.packChecks {
      if let pack = packs.packs.first(where: { $0.packId == pc.packId }) {
        let hits = pack.signatureTokens.reduce(0) { acc, t in
          let needle = StudioNormV1.normNameV1(t.expectContains)
          let ok = inv.items.flatMap { $0.evidence.samples }.contains { s in s.confidence >= pack.confidenceThreshold && StudioNormV1.normNameV1(s.ocrText).contains(needle) }
          return acc + (ok ? 1 : 0)
        }
