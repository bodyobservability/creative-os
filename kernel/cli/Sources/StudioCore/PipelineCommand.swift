...
struct Pipeline: AsyncParsableCommand {
  ...
  struct CutProfile: AsyncParsableCommand {
    ...
    @Option(name: .long, help: "Baseline path (canonical). If omitted, uses shared/specs/profiles/<active_profile>/sonic/baselines/<rack>/<macro>.baseline.v1.json")
    var baseline: String?
    ...
    func run() async throws {
      ...
      print("receipt: \(RepoPaths.defaultRunsDir())/\(receipt.runId)/release_cut_receipt.v1.json")
