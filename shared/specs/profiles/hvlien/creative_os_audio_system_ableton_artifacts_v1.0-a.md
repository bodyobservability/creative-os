...
## SPEC v1.0-A (EXTENSION) This document specifies the required Ableton artifacts and their canonical structure.  ## 1. ABLETON ARTIFACT INVENTORY (REQUIRED) ### Ableton Project Files
 `operator/packs/hvlien-defaults/ableton/finishing-bays/HVLIEN_FINISHING_BAY.als`
 `operator/packs/hvlien-defaults/ableton/finishing-bays/HVLIEN_FINISHING_BAY_DARK.als`
 `operator/packs/hvlien-defaults/ableton/finishing-bays/HVLIEN_FINISHING_BAY_LIGHT.als`  ## 2. GLOBAL PREFERENCES (MUST MATCH) | Setting | Value |
|-|-|
| Sample Rate | 48 kHz |
...
`HVLIEN_FINISHING_BAY.als` ### Track Layout (Exact)
 ...
 Warp OFF
 No edits, automation, fades, clip gain ### MUSIC_BUS Device Stack (fixed order)
...
5. Limiter: Ceiling -1.0 dBFS, minimal gain ### Export preset
 WAV, 24-bit, 48kHz
 Normalize OFF
 Dither OFF  ## 4. DARK BAY — DELTA ### File
`HVLIEN_FINISHING_BAY_DARK.als` Allowed deltas only:
 Saturator Drive: +0.5 to +1 dB vs Primary
 Glue Release: ~300 ms (fixed)
 Utility Width: 95–98%
 Limiter gain: slightly reduced No structural changes.  ## 5. LIGHT BAY — DELTA ### File
`HVLIEN_FINISHING_BAY_LIGHT.als` Allowed deltas only:
 Saturator Drive: -0.5 to -1 dB vs Primary
 Glue Release: ~120 ms
 Utility Width: 102–105%
 Slightly more headroom (less limiter input) No structural changes.  ## 6. VERSIONING Allowed:
 value tuning (thresholds, timing constants, small gain offsets) Forbidden:
 routing changes
 device insertion/removal
 per-track edits Structural change requires bump:
 `..._v1.1.als`  # END SPEC v1.0-A
