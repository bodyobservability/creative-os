...
- Global Ableton prefs set per `shared/specs/profiles/hvlien/creative_os_audio_system_ableton_artifacts_v1.0-a.md`
...
shared/specs/profiles/hvlien/creative_os_audio_system_ableton_artifacts_v1.0-a.md
...
Follow `shared/specs/profiles/hvlien/creative_os_audio_system_ableton_artifacts_v1.0-a.md`, Section 3.
...
1. Save each `.als` into:
   - `operator/packs/hvlien-defaults/ableton/finishing-bays/`
   - Overwrite the placeholder file.
2. Recompute checksums:
   ```bash
   kernel/tools/checksum_generate.sh
   ```
3. Verify:
   ```bash
   kernel/tools/checksum_verify.sh
   ```
4. Commit:
   ```bash
   git add .
